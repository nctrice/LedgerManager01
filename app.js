...
function refreshLedgers(){
  ...
  Store.getLedgers().forEach(l=>{ const span=document.createElement('span'); span.className='pill'; span.textContent=l; list.appendChild(span); });
}
...
addLedgerBtn.addEventListener('click', ()=>{
  ...
  document.getElementById('new-ledger-name').value=''; refreshLedgers(); refreshDashboard(); refreshPaymentsTable(); refreshReceivables(); refreshStockTable();
});
...
document.querySelectorAll('input[name="pay-kind"]').forEach(r=>{
  r.addEventListener('change', ()=>{
    if(r.checked){ const v=r.value; creditFields.classList.toggle('hidden', v!=='credit'); debitFields.classList.toggle('hidden', v!=='debit'); }
  });
});
...
addCreditBtn.addEventListener('click', ()=>{
  ...
  Store.addCredit({ date, name, paymentType, amount, ledger }); document.getElementById('credit-name').value=''; document.getElementById('credit-amount').value='';
...
addDebitBtn.addEventListener('click', ()=>{
  const ledger = ledgerSelects.payment.value; const date = (document.getElementById('payment-date').value || new Date().toISOString().slice(0,10));
  const invoiceType = document.getElementById('debit-invoice-type').value.trim(); const invoiceNumber = document.getElementById('debit-invoice-number').value.trim();
  const amount = Number(document.getElementById('debit-amount').value);
  if(!ledger){ alert('Select a ledger'); return; } if(!amount){ alert('Enter amount'); return; }
  Store.addDebit({ date, invoiceType, invoiceNumber, amount, ledger }); document.getElementById('debit-invoice-type').value=''; document.getElementById('debit-invoice-number').value=''; document.getElementById('debit-amount').value='';
  refreshPaymentsTable(); refreshDashboard();
});
...
function refreshPaymentsTable(){
  const ledger = paymentsFilter.value || 'All'; const rows = Store.listPayments(ledger); const wrap = document.getElementById('payments-table');
  let html = '<table><thead><tr><th>Date</th><th>Type</th><th>Details</th><th>Ledger</th><th>Amount</th><th></th></tr></thead><tbody>';
  for(const r of rows){ let details=''; if(r.type==='credit') details = `${r.name||''} · ${r.paymentType||''}`; else details = `${r.invoiceType||''} · #${r.invoiceNumber||''}`;
...
exportBtn.addEventListener('click', ()=>{
  const ledger = (paymentsFilter.value==='All' || !paymentsFilter.value) ? ledgerSelects.payment.value : paymentsFilter.value; if(!ledger){ alert('Select a ledger to export'); return; }
...
// Receivables
const addRecvBtn = document.getElementById('add-recv-btn'); addRecvBtn.addEventListener('click', ()=>{
  const ledger = ledgerSelects.recv.value; const date = (document.getElementById('recv-date').value || new Date().toISOString().slice(0,10));
  const customer = document.getElementById('recv-customer').value.trim(); const invoiceNumber = document.getElementById('recv-invnum').value.trim(); const amount = Number(document.getElementById('recv-amount').value); const comment = document.getElementById('recv-comment').value.trim();
  if(!ledger){ alert('Select a ledger'); return; } if(!customer){ alert('Enter customer'); return; } if(!amount){ alert('Enter amount'); return; }
  Store.addReceivable({ date, customer, invoiceNumber, amount, comment, ledger }); document.getElementById('recv-customer').value=''; document.getElementById('recv-invnum').value=''; document.getElementById('recv-amount').value=''; document.getElementById('recv-comment').value='';
...
function refreshReceivables(){
  const ledger = ledgerSelects.recvFilter.value || 'All'; const rows = Store.listReceivables(ledger); const wrap = document.getElementById('receivables-table'); const ledgers = Store.getLedgers();
  ...
  for(const r of rows){ const selId = `move-${r.id}`; let sel = `<select id="${selId}">`; for(const l of ledgers){ sel += `<option value="${l}" ${l===r.ledger?'selected':''}>${l}</option>`; } sel+='</select>';
...
function refreshFavouriteSections(){ const favs = Store.getFavourites(); const box = document.getElementById('fav-recv-container'); box.innerHTML=''; if(favs.length===0){ box.innerHTML = '<p class="small">No favourites yet. Add up to 5 customers to track their receivables separately.</p>'; return; } const all = Store.listReceivables('All'); for(const name of favs){ const rows = all.filter(r=>r.customer===name); let html = `<div class="card" style="margin:.5rem 0"><h4>${name}</h4><div class="table">`; if(rows.length===0){ html += `<p class="small">No receivables yet.</p>`; } else { html += '<table><thead><tr><th>Date</th><th>Invoice #</th><th>Ledger</th><th>Amount</th><th>Comment</th></tr></thead><tbody>'; for(const r of rows){ html += `<tr><td>${r.date||''}</td><td>${r.invoiceNumber||''}</td><td>${r.ledger}</td><td>${naira(r.amount)}</td><td>${r.comment||''}</td></tr>`; } html += '</tbody></table>'; } const total = rows.reduce((s,r)=>s+Number(r.amount||0),0); html += `<div class="pill" style="margin-top:.5rem">Total: ${naira(total)}</div>`; html += '</div></div>'; const div=document.createElement('div'); div.innerHTML = html; box.appendChild(div); } }
