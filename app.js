...
// Tabs: default to Payments on (re)load
...
function refreshLedgers(){
  ...
  Object.values(ledgerSelects).forEach(sel=>{
    ...
    sel.innerHTML='';
    if(sel===ledgerSelects.paymentsFilter || sel===ledgerSelects.recvFilter){
...
addLedgerBtn.addEventListener('click', ()=>{
  ...
  const res = Store.addLedger(name);
...
document.querySelectorAll('input[name="pay-kind"]').forEach(r=>{
  r.addEventListener('change', ()=>{
    if(r.checked){
      const v = r.value;
      creditFields.classList.toggle('hidden', v!== 'credit');
      debitFields.classList.toggle('hidden', v!== 'debit');
    }
  });
});
...
addCreditBtn.addEventListener('click', ()=>{
  const ledger = ledgerSelects.payment.value;
...
addDebitBtn.addEventListener('click', ()=>{
  const ledger = ledgerSelects.payment.value;
  const date = (document.getElementById('payment-date').value || new Date().toISOString().slice(0,10));
  const invoiceType = document.getElementById('debit-invoice-type').value.trim();
  const invoiceNumber = document.getElementById('debit-invoice-number').value.trim();
  ...
  document.getElementById('debit-invoice-type').value='';
  document.getElementById('debit-invoice-number').value='';
...
paymentsFilter.addEventListener('change', refreshPaymentsTable);
...
function refreshPaymentsTable(){
  ...
  for(const r of rows){
    let details='';
    if(r.type==='credit') details = `${r.name||''} · ${r.paymentType||''}`;
    else details = `${r.invoiceType||''} · #${r.invoiceNumber||''}`;
    html += `<tr><td>${r.date||''}</td><td>${r.type}</td><td>${details}</td>`+
            `<td>${r.ledger}</td><td>${naira(r.amount)}</td>`+
            `<td><button class="danger" data-del="${r.id}">Delete</button></td></tr>`;
  }
  html += '</tbody></table>';
  wrap.innerHTML = html;
...
// Export CSV for selected ledger
...
exportBtn.addEventListener('click', ()=>{
  ...
  for(const r of rows){
    if(r.type==='credit'){
      lines.push([r.date, 'Credit', (r.name||''), (r.paymentType||''), r.ledger, r.amount].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','));
    } else {
      lines.push([r.date, 'Debit', (r.invoiceType||''), (r.invoiceNumber||''), r.ledger, r.amount].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','));
    }
  }
  const blob = new Blob([lines.join('\n')], { type:'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `ledger_${ledger}_statement.csv`;
...
addRecvBtn.addEventListener('click', ()=>{
  const ledger = ledgerSelects.recv.value;
  const date = (document.getElementById('recv-date').value || new Date().toISOString().slice(0,10));
  const customer = document.getElementById('recv-customer').value.trim();
  const invoiceNumber = document.getElementById('recv-invnum').value.trim();
  const amount = Number(document.getElementById('recv-amount').value);
  const comment = document.getElementById('recv-comment').value.trim();
  if(!ledger){ alert('Select a ledger'); return; }
  if(!customer){ alert('Enter customer'); return; }
  if(!amount){ alert('Enter amount'); return; }
  Store.addReceivable({ date, customer, invoiceNumber, amount, comment, ledger });
  document.getElementById('recv-customer').value='';
  document.getElementById('recv-invnum').value='';
  document.getElementById('recv-amount').value='';
  document.getElementById('recv-comment').value='';
  refreshReceivables();
  refreshDashboard();
});
...
function refreshReceivables(){
  ...
  for(const r of rows){
    ...
    sel += '</select>';
    html += `<tr><td>${r.date||''}</td><td>${r.customer||''}</td><td>${r.invoiceNumber||''}</td>`+
            `<td>${r.ledger}</td><td>${naira(r.amount)}</td><td>${r.comment||''}</td>`+
            `<td>${sel} <button class="outline" data-move="${r.id}">Move</button></td>`+
            `<td><button class="danger" data-del="${r.id}">Delete</button></td></tr>`;
  }
  html += '</tbody></table>';
  wrap.innerHTML = html;
  wrap.querySelectorAll('button[data-del]').forEach(btn=>{
    btn.addEventListener('click', ()=>{ if(confirm('Delete receivable?')){ Store.deleteReceivable(btn.dataset.del); refreshReceivables(); refreshDashboard(); } });
  });
  wrap.querySelectorAll('button[data-move]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const id = btn.dataset.move;
      const sel = document.getElementById(`move-${id}`);
      const newLedger = sel.value;
      Store.moveReceivableLedger(id, newLedger);
      refreshReceivables();
      refreshDashboard();
    });
  });
...
function renderFavourites(){
  const favWrap = document.getElementById('fav-customers');
  favWrap.innerHTML='';
...
addFavBtn.addEventListener('click', ()=>{
  const name = document.getElementById('new-fav-name').value;
  const res = Store.addFavourite(name);
  if(!res.ok){ alert(res.msg); return; }
...
function refreshFavouriteSections(){
  ...
  const box = document.getElementById('fav-recv-container');
  box.innerHTML = '';
  ...
  for(const name of favs){
    ...
    if(rows.length===0){ html += `<p class="small">No receivables yet.</p>`; }
      ...
      for(const r of rows){
        html += `<tr><td>${r.date||''}</td><td>${r.invoiceNumber||''}</td><td>${r.ledger}</td><td>${naira(r.amount)}</td><td>${r.comment||''}</td></tr>`;
      }
      html += '</tbody></table>';
    }
...
function populateProducts(){
  ...
  for(const [name, price] of Object.entries(products)){
    const opt = document.createElement('option'); opt.value=name; opt.textContent=name; opt.dataset.price = price; productSelect.appendChild(opt);
  }
...
function refreshStockTable(){
  const ledger = ledgerSelects.stock.value;
  const items = Store.getStockForLedger(ledger);
  const wrap = document.getElementById('stock-table');
  let html = '<table><thead><tr><th>Product</th><th>Price (₦)</th><th>Qty</th><th>Value</th></tr></thead><tbody>';
  for(const it of items){
    html += `<tr><td>${it.name}</td><td>${naira(it.price)}</td><td>${it.qty}</td><td>${naira(it.value)}</td></tr>`;
  }
  html += '</tbody></table>';
  wrap.innerHTML = html;
  document.getElementById('stock-total').textContent = `Total Value: ${naira(items.reduce((s,x)=>s+x.value,0))}`;
  refreshDashboard();
}
...
addStockBtn.addEventListener('click', ()=>{
  ...
  Store.setStock(ledger, product, price, qty);
...
function refreshDashboard(){
  ...
  const worth = debits - (credits + recv + stock);
  ...
  w.textContent = naira(worth);
  w.style.background = worth>0 ? '#065f46' : worth<0 ? '#7f1d1d' : '#6b7280';
}
