...
const Store = (() => {
  ...
  function load(){
    try{
      return JSON.parse(localStorage.getItem(KEY)) || {
        ledgers: ['General'],
        payments: [], // {id, type:'credit'|'debit', date, name?, paymentType?, amount, invoiceType?, invoiceNumber?, ledger, createdAt}
        receivables: [], // {id, date, customer, invoiceNumber, amount, comment, ledger, createdAt}
        favoriteCustomers: [],
        stock: { // per ledger
  ...
  function ensureLedgerStock(ledger){
    if(!state.stock[ledger]){
      state.stock[ledger] = { products: {} };
    }
    // Ensure all known products exist with default price if missing
  ...
  return {
    ...
    paymentsTotals: (ledger) => {
      ...
      for(const p of arr){ if(p.type==='credit') credits += Number(p.amount||0); else debits += Number(p.amount||0); }
    ...
    deleteReceivable: (id) => {
      ...
      if(idx>=0){ state.receivables.splice(idx,1); save(state); }
    },
    moveReceivableLedger: (id, newLedger) => {
      const it = state.receivables.find(r=>r.id===id);
      if(it){ it.ledger = newLedger; save(state); }
    },
    ...
    receivablesTotal: (ledger) => {
      return state.receivables.filter(r=>r.ledger===ledger).reduce((s,r)=>s+Number(r.amount||0),0);
    },
    ...
    removeFavourite: (name)=>{
      state.favoriteCustomers = state.favoriteCustomers.filter(n=>n!==name); save(state);
    },

    // STOCK (per ledger)
    listProducts: ()=> Object.assign({}, defaultProducts),
    setStock: (ledger, product, price, qty) => {
      ...
      state.stock[ledger].products[product] = p;
      save(state);
    },
    getStockForLedger: (ledger) => {
      ensureLedgerStock(ledger);
      const items = state.stock[ledger].products || {};
      return Object.entries(items).map(([name, v])=>({ name, price: Number(v.price||0), qty: Number(v.qty||0), value: Number(v.price||0) * Number(v.qty||0) }));
    },
    stockTotalValue: (ledger) => {
      return (Store.getStockForLedger(ledger)||[]).reduce((s,it)=>s+it.value,0);
    }
  };
})();
