const Store = (()=>{ const KEY='ledgerpwa.v1';
  function load(){
    ...
    for (var k in defaults){ defStock.products[k] = { price: defaults[k], qty: 0 }; }
    return { ledgers:['General'], payments:[], receivables:[], favoriteCustomers:[], stock:{ 'General': defStock } };
  }
  function save(s){ localStorage.setItem(KEY, JSON.stringify(s)); }
  const state = load();
  function ensureLedgerStock(ledger){ if(!state.stock[ledger]) state.stock[ledger]={products:{}}; var defaults={ 'Tydineal cream':700,'Tydisil Cream':945,'Anofast Gel':700,'Tydiclear Cream':515,'Tydibact Cream':580,'Borocare Cream':1520,'Klaract Cream':1565,'Ciprofloxacin Tablet':400,'Metformin Tablet':800,'Vitamin C Syrup':300,'Gentamicin Inj':5995,'Diclofenac Inj':4550,'Artemether Inj':4350,'M&B Isopropyl Alcohol':950 }; for(var k in defaults){ if(!state.stock[ledger].products[k]) state.stock[ledger].products[k] = { price: defaults[k], qty: 0 }; } }
  return {
    getLedgers: function(){ return Array.from(state.ledgers||[]); },
    addLedger: function(name){ var nm=String(name||'').trim(); if(!nm) return {ok:false,msg:'Enter a name'}; if(!state.ledgers) state.ledgers=[]; for(var i=0;i<state.ledgers.length;i++){ if(state.ledgers[i]===nm) return {ok:false,msg:'Ledger already exists'}; } state.ledgers.push(nm); ensureLedgerStock(nm); save(state); return {ok:true}; },
    // PAYMENTS
    ...
    getStockForLedger: function(ledger){ ensureLedgerStock(ledger); var items=state.stock[ledger].products||{}; var out=[]; for(var name in items){ var v=items[name]; var price=Number(v.price||0), qty=Number(v.qty||0); out.push({name:name,price:price,qty:qty,value:price*qty}); } return out; },
    ...
    clearTransactionsAndStock: function(){ state.payments=[]; state.receivables=[]; for(var i=0;i<state.ledgers.length;i++){ var l=state.ledgers[i]; ensureLedgerStock(l); var prods=state.stock[l].products||{}; for(var k in prods){ prods[k].qty=0; } state.stock[l].products=prods; } save(state); }
  };
})();
